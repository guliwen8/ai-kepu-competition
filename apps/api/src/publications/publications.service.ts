...
import { JudgingAssignmentStatus, SubmissionCategory, SubmissionStatus } from '@prisma/client';
...
export class PublicationsService {
  ...
  private aggregateScores(rows: Array<{ submissionId: string; score: any }>) {
    const map = new Map<
      string,
      { count: number; total: number; s1: number; s2: number; s3: number; s4: number; s5: number }
    >();
    for (const r of rows) {
      const s = r.score;
      if (!s) continue;
      const cur = map.get(r.submissionId) ?? { count: 0, total: 0, s1: 0, s2: 0, s3: 0, s4: 0, s5: 0 };
      cur.count += 1;
  ...
  private withRank(items: Array<{ score: ScoreAgg; blindCode: string; title: string | null; category: string; submissionId: string }>) {
    const ranked: Array<
      { rank: number } & { score: ScoreAgg; blindCode: string; title: string | null; category: string; submissionId: string }
    > = [];
  ...
  async getPublicLeaderboard(args: {
    ...
    const where: any = { competitionId: competition.id, status: SubmissionStatus.PUBLICIZED };
    ...
    const assignments = submissionIds.length
      ? await this.prisma.judgingAssignment.findMany({
          where: { submissionId: { in: submissionIds }, status: JudgingAssignmentStatus.SUBMITTED },
          select: { submissionId: true, score: true },
        })
      : [];

    const scores = this.aggregateScores(assignments).filter((s) => s.scoreCount >= minScoreCount);
    ...
    const merged = submissions
      ...
      .filter(Boolean) as Array<{ submissionId: string; blindCode: string; title: string | null; category: string; score: ScoreAgg }>;

    merged.sort((a, b) => {
      if (b.score.avgTotal !== a.score.avgTotal) return b.score.avgTotal - a.score.avgTotal;
      if (b.score.scoreCount !== a.score.scoreCount) return b.score.scoreCount - a.score.scoreCount;
    ...
    return {
      competition: { id: competition.id, title: competition.title, phase: competition.phase },
  ...
  async getPublicSubmissionDetail(args: { submissionId: string }) {
    ...
    if (submission.status !== SubmissionStatus.PUBLICIZED) throw new NotFoundException('作品不存在');
    const competition = submission.competition;
    if (!competition) throw new NotFoundException('赛事不存在');
    const phase = calcCompetitionPhase(competition as any);
    this.assertPublicPhase(phase);

    const minScoreCount = this.minScoreCountFromConfig((competition as any).config);
    const assignments = await this.prisma.judgingAssignment.findMany({
      where: { submissionId: submission.id, status: JudgingAssignmentStatus.SUBMITTED },
      select: { submissionId: true, score: true },
    });
    const scores = this.aggregateScores(assignments).filter((s) => s.scoreCount >= minScoreCount);
  ...
  async adminExportLeaderboardCsv(args: { competitionId?: string; category?: SubmissionCategory }) {
    ...
    const header = ['rank', 'blindCode', 'submissionId', 'category', 'title', 'avgTotal', 'scoreCount', 'avgS1', 'avgS2', 'avgS3', 'avgS4', 'avgS5'];
