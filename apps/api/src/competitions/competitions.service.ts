...
function validateCompetitionConfig(config: unknown) {
  ...
  if (cfg.allowedCategories != null) {
    if (!Array.isArray(cfg.allowedCategories))
      throw new BadRequestException('配置不合法：allowedCategories 必须是数组');
    for (const c of cfg.allowedCategories) {
      if (typeof c !== 'string' || !submissionCategories.has(c)) {
        throw new BadRequestException(
          '配置不合法：allowedCategories 包含未知类别',
        );
      }
    }
  }
  ...
  if (cfg.categoryOptions != null) {
    if (!Array.isArray(cfg.categoryOptions))
      throw new BadRequestException('配置不合法：categoryOptions 必须是数组');
    for (const it of cfg.categoryOptions) {
      if (!it || typeof it !== 'object' || Array.isArray(it))
        throw new BadRequestException(
          '配置不合法：categoryOptions 元素必须是对象',
        );
      if (typeof it.label !== 'string' || !it.label.trim()) {
        throw new BadRequestException(
          '配置不合法：categoryOptions.label 必须是非空字符串',
        );
      }
      if (typeof it.value !== 'string' || !submissionCategories.has(it.value)) {
        throw new BadRequestException(
          '配置不合法：categoryOptions.value 必须是合法 SubmissionCategory',
        );
      }
    }
  }
  ...
  if (cfg.registrationFields != null) {
    if (
      !cfg.registrationFields ||
      typeof cfg.registrationFields !== 'object' ||
      Array.isArray(cfg.registrationFields)
    ) {
      throw new BadRequestException(
        '配置不合法：registrationFields 必须是对象',
      );
    }
    ...
    for (const [k, v] of Object.entries(cfg.registrationFields)) {
      if (!allowedKeys.has(k))
        throw new BadRequestException(
          '配置不合法：registrationFields 包含未知字段',
        );
      if (v === false) continue;
      if (!v || typeof v !== 'object' || Array.isArray(v))
        throw new BadRequestException(
          '配置不合法：registrationFields 字段配置必须是对象或 false',
        );
      if ('enabled' in (v as any) && typeof (v as any).enabled !== 'boolean') {
        throw new BadRequestException(
          '配置不合法：registrationFields.enabled 必须是 boolean',
        );
      }
      if (
        'required' in (v as any) &&
        typeof (v as any).required !== 'boolean'
      ) {
        throw new BadRequestException(
          '配置不合法：registrationFields.required 必须是 boolean',
        );
      }
      if ('label' in (v as any) && typeof (v as any).label !== 'string') {
        throw new BadRequestException(
          '配置不合法：registrationFields.label 必须是 string',
        );
      }
      if (
        'placeholder' in (v as any) &&
        typeof (v as any).placeholder !== 'string'
      ) {
        throw new BadRequestException(
          '配置不合法：registrationFields.placeholder 必须是 string',
        );
      }
      if ('hint' in (v as any) && typeof (v as any).hint !== 'string') {
        throw new BadRequestException(
          '配置不合法：registrationFields.hint 必须是 string',
        );
      }
      if (
        'maxLength' in (v as any) &&
        typeof (v as any).maxLength !== 'number'
      ) {
        throw new BadRequestException(
          '配置不合法：registrationFields.maxLength 必须是 number',
        );
      }
    }
  }
  ...
  if (cfg.materialRequirements != null) {
    if (
      !cfg.materialRequirements ||
      typeof cfg.materialRequirements !== 'object' ||
      Array.isArray(cfg.materialRequirements)
    ) {
      throw new BadRequestException(
        '配置不合法：materialRequirements 必须是对象',
      );
    }
    for (const [cat, req] of Object.entries(cfg.materialRequirements)) {
      if (!submissionCategories.has(cat))
        throw new BadRequestException(
          '配置不合法：materialRequirements 包含未知类别',
        );
      if (!req || typeof req !== 'object' || Array.isArray(req))
        throw new BadRequestException(
          '配置不合法：materialRequirements[category] 必须是对象',
        );
      const requiredKinds = (req as any).requiredKinds;
      const rules = (req as any).rules;
      if (!Array.isArray(requiredKinds))
        throw new BadRequestException('配置不合法：requiredKinds 必须是数组');
      if (!Array.isArray(rules))
        throw new BadRequestException('配置不合法：rules 必须是数组');
      for (const k of requiredKinds) {
        if (typeof k !== 'string' || !attachmentKinds.has(k))
          throw new BadRequestException(
            '配置不合法：requiredKinds 包含未知 kind',
          );
      }
      for (const r of rules) {
        if (!r || typeof r !== 'object' || Array.isArray(r))
          throw new BadRequestException('配置不合法：rules 元素必须是对象');
        if (typeof r.kind !== 'string' || !attachmentKinds.has(r.kind)) {
          throw new BadRequestException(
            '配置不合法：rules.kind 必须是合法 AttachmentKind',
          );
        }
        if ('durationSecMin' in r || 'durationSecMax' in r) {
          if (
            typeof r.durationSecMin !== 'number' ||
            typeof r.durationSecMax !== 'number'
          ) {
            throw new BadRequestException(
              '配置不合法：durationSecMin/durationSecMax 必须是 number',
            );
          }
        }
        if ('maxBytes' in r && typeof r.maxBytes !== 'number') {
          throw new BadRequestException('配置不合法：maxBytes 必须是 number');
        }
        if ('minBytes' in r && typeof r.minBytes !== 'number') {
          throw new BadRequestException('配置不合法：minBytes 必须是 number');
        }
        if ('mimeTypes' in r) {
          if (
            !Array.isArray(r.mimeTypes) ||
            r.mimeTypes.some((m: any) => typeof m !== 'string')
          ) {
            throw new BadRequestException(
              '配置不合法：mimeTypes 必须是 string[]',
            );
          }
        }
      }
  ...
  if (cfg.privacyConfirmation != null) {
    if (
      !cfg.privacyConfirmation ||
      typeof cfg.privacyConfirmation !== 'object' ||
      Array.isArray(cfg.privacyConfirmation)
    ) {
      throw new BadRequestException(
        '配置不合法：privacyConfirmation 必须是对象',
      );
    }
    if (
      'enabled' in cfg.privacyConfirmation &&
      typeof cfg.privacyConfirmation.enabled !== 'boolean'
    ) {
      throw new BadRequestException(
        '配置不合法：privacyConfirmation.enabled 必须是 boolean',
      );
    }
    if (
      'text' in cfg.privacyConfirmation &&
      typeof cfg.privacyConfirmation.text !== 'string'
    ) {
      throw new BadRequestException(
        '配置不合法：privacyConfirmation.text 必须是 string',
      );
    }
  }
}
...
export function calcCompetitionPhase(c: {
  ...
  const latestEnd = [c.publicEnd, c.judgingEnd, c.submissionEnd]
    .filter(Boolean)
    .map((d) => (d as Date).getTime());
  if (latestEnd.length > 0 && now > Math.max(...latestEnd))
...
export class CompetitionsService {
  ...
  async create(input: {
    ...
    const submissionStart = parseDate(input.submissionStart) as
      | Date
      | undefined;
    ...
    const hasCurrent = await this.prisma.competition.count({
    ...
    return { ...created, phase: calcCompetitionPhase(created) };
  }
  ...
  async update(id: string, input: any) {
    ...
    if (Object.prototype.hasOwnProperty.call(input, 'config')) {
      ...
      patch.config = v === null ? null : (v ?? undefined);
    }

    const merged = { ...existing, ...patch };
    if (
      merged.submissionStart &&
      merged.submissionEnd &&
      merged.submissionStart > merged.submissionEnd
    ) {
      throw new BadRequestException('提交开始时间不能晚于结束时间');
    }
    if (
      merged.judgingStart &&
      merged.judgingEnd &&
      merged.judgingStart > merged.judgingEnd
    ) {
      throw new BadRequestException('评审开始时间不能晚于结束时间');
    }
    if (
      merged.publicStart &&
      merged.publicEnd &&
      merged.publicStart > merged.publicEnd
    ) {
      throw new BadRequestException('公示开始时间不能晚于结束时间');
    }
    ...
    return { ...updated, phase: calcCompetitionPhase(updated) };
  }
  ...
  async setCurrent(id: string) {
    ...
    await this.prisma.$transaction([
      this.prisma.competition.updateMany({
        where: { isCurrent: true },
  ...
  async getCurrent() {
    ...
    return c ? { ...c, phase: calcCompetitionPhase(c) } : null;
  }
}
