...
export class SubmissionsService {
  ...
  private assertRegistrationFields(
    ...
    const introOk =
      typeof args.intro === 'string' && args.intro.trim().length > 0;
    const aiOk =
      typeof args.aiToolsUsage === 'string' &&
      args.aiToolsUsage.trim().length > 0;
    const nameOk =
      typeof args.teacherName === 'string' &&
      args.teacherName.trim().length > 0;
    const contactOk =
      typeof args.teacherContact === 'string' &&
      args.teacherContact.trim().length > 0;
    if (flags.intro && !introOk)
      throw new BadRequestException(`请填写作品简介后再${action}`);
    if (flags.aiToolsUsage && !aiOk)
      throw new BadRequestException(`请填写 AI 工具使用说明后再${action}`);
    if (flags.teacherName && !nameOk)
      throw new BadRequestException(`请填写指导老师姓名后再${action}`);
    if (flags.teacherContact && !contactOk)
      throw new BadRequestException(`请填写指导老师联系方式后再${action}`);
  }

  private assertSubmissionWindow(
    args: { submissionStart: Date | null; submissionEnd: Date | null },
    action: string,
  ) {
    const now = Date.now();
  ...
  private assertEditableWindow(
    args: { submissionEnd: Date | null },
    action: string,
  ) {
    const now = Date.now();
    if (args.submissionEnd && now > args.submissionEnd.getTime()) {
  ...
  async createDraft(
    ...
    if (count >= 3)
      throw new BadRequestException('每位参赛者最多提交 3 件作品');

    const currentCompetition = await this.prisma.competition.findFirst({
      where: { isCurrent: true },
    });
    if (currentCompetition) {
      this.assertEditableWindow(
        { submissionEnd: currentCompetition.submissionEnd },
        '创建作品',
      );
    }
    ...
    let membersToSnapshot: Array<{ userId: string; role?: string }> = [
      { userId, role: 'owner' },
    ];
    ...
    if (input.teamId) {
      ...
      if (team.members.length > 10)
        throw new BadRequestException('团队人数上限为 10 人');
      ...
      membersToSnapshot = team.members.map((m) => ({
        userId: m.userId,
        role: m.role,
      }));
    }
    ...
    const submission = await this.prisma.submission.create({
      data: {
        ...
        teamId: input.teamId,
        members: {
          create: membersToSnapshot.map((m) => ({
            userId: m.userId,
            role: m.role ?? 'member',
          })),
        },
  ...
  async getById(userId: string, id: string) {
    ...
    const canRead =
      ...
      submission.members.some((m) => m.userId === userId);
  ...
  async updateDraft(userId: string, id: string, patch: any) {
    ...
    const editable =
      ...
      submission.status === SubmissionStatus.NEED_FIX;
    if (!editable) {
      throw new BadRequestException('当前状态不可编辑');
    }
    if (submission.competition) {
      this.assertEditableWindow(
        { submissionEnd: submission.competition.submissionEnd },
        '编辑作品',
      );
    }
    if (
      'intro' in patch ||
      'aiToolsUsage' in patch ||
      'teacherName' in patch ||
      'teacherContact' in patch
    ) {
      this.assertRegistrationFields(
        {
          intro: patch.intro,
  ...
  async submit(userId: string, id: string) {
    ...
    if (submission.competition) {
      this.assertSubmissionWindow(
        {
          submissionStart: submission.competition.submissionStart,
          submissionEnd: submission.competition.submissionEnd,
        },
        '提交作品',
      );
    }
    this.assertAllowedCategory(
      submission.category,
      (submission as any).competition?.config,
    );
    this.assertRegistrationFields(
    ...
    const { pass, findings } = this.formatCheck(
    ...
    let nextStatus: SubmissionStatus = pass
    ...
    if (pass) {
      ...
      const combinedFindings: AnonymityFinding[] = [
        ...ruleResult.findings,
        ...llmFindings,
      ];
      ...
      if (anonTaskStatus !== ReviewTaskStatus.FAIL) {
        ...
        else if (contentStatus === 'FAIL')
          contentTaskStatus = ReviewTaskStatus.FAIL;
        else if (contentStatus === 'NEED_MANUAL')
